...
export const NOAUTH_PROVIDERS = {
  ...
  auggie: {
    ...
    isLocalCli: true,
    freeNote:
      "Local passthrough — runs the Augment CLI (`auggie`) on this machine. Auth is handled by `auggie login`, not RouteChi.",
    authHint:
      "No API key stored by RouteChi. Install the Auggie CLI and run `auggie login` on this machine, then RouteChi spawns it locally for each request.",
    notice: {
      text: "Augment (Auggie CLI) requires the `auggie` binary installed and authenticated locally (`auggie login`). RouteChi spawns it as a subprocess and never sees or stores your Augment credentials.",
    },
  },
  "zai-web-free": {
    id: "zai-web-free",
