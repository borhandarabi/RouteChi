...
/**
 * Build the `X-RouteChi-Decision` composite header value: `strategy=<name>;
 ...
 * (same alias normalization the `X-RouteChi-Provider` header already applies)
...
export function buildRouteChiResponseMetaHeaders({
  ...
  if (costSavedUsd != null) {
    headers[OMNIROUTE_RESPONSE_HEADERS.costSaved] = toHeaderValue(formatRouteChiCost(costSavedUsd));
  }
