...
export default function DefaultToolCard({
  ...
  const resolveApiKeyValue = useCallback(
    () => selectedKeyObj?.rawKey || (!cloudEnabled ? "sk-YOUR-API-KEY" : t("yourApiKeyPlaceholder")),
  ...
  useEffect(() => {
    const savedModel = localStorage.getItem(`routechi-cli-model-${toolId}`);
    if (savedModel) {
      if (isMultiModelTool) {
        try {
    ...
    const savedKey = localStorage.getItem(`routechi-cli-key-${toolId}`);
  ...
  const handleModelChange = useCallback(
    (value) => {
      setModelValue(value);
      if (value) {
        localStorage.setItem(`routechi-cli-model-${toolId}`, value);
      } else {
        localStorage.removeItem(`routechi-cli-model-${toolId}`);
      }
    },
    [toolId]
  );
  ...
  const handleModelValuesChange = useCallback(
    (values) => {
      ...
      if (normalized.length > 0) {
        localStorage.setItem(`routechi-cli-model-${toolId}`, JSON.stringify(normalized));
      } else {
        localStorage.removeItem(`routechi-cli-model-${toolId}`);
      }
    },
  ...
  const handleApiKeyChange = useCallback(
    (value) => {
      ...
      if (value) {
        ...
        localStorage.setItem(`routechi-cli-key-${toolId}`, value);
      }
    },
    [toolId]
  );
      ...
      <div className="mt-2 flex items-center gap-2">
        {apiKeys && apiKeys.length > 0 ? (
          ...
          <span className="text-sm text-text-muted">
            {cloudEnabled ? t("noApiKeysCreateOne") : "sk-YOUR-API-KEY"}
          </span>
        )}
      </div>
    );
