...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (allowed.length > 0) {
      ...
      if (!ok) {
        return NextResponse.redirect(new URL("/login?oidc_error=subject_not_allowed", reqUrlEarly));
      }
    }
  } catch {
  ...
  if (!process.env.JWT_SECRET) {
    return NextResponse.redirect(new URL("/login?oidc_error=server_misconfigured", reqUrlEarly));
  }
