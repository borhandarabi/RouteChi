 ...
 *      RouteChi itself, OpenRouter, a Gemini-OpenAI-compat URL).
 ...
 *      RouteChi's own `/v1` so the vision model can be any provider
 *      registered in RouteChi (`google/gemini-2.0-flash`,
 *      `openrouter/...`, etc.) instead of being limited to OpenAI/Anthropic.
 *   2. `OPENAI_API_URL` env var (legacy)
 *   3. RouteChi self-loop (`http://localhost:20128/v1`) — auto-detected when
 *      the model uses a known RouteChi-internal provider (e.g. `kr/`, `if/`,
 ...
 *                that require RouteChi self-loop routing.
 */
export function resolveVisionBridgeBaseUrl(model?: string): string {
  ...
  // anthropic/), it can only be resolved through RouteChi's own router,
...
async function callVisionModelSingle(
  ...
  try {
    ...
    if (isAnthropic) {
      ...
      // RouteChi itself or any other OpenAI-compatible endpoint instead of
      // hardcoded api.openai.com.
      const baseUrl = resolveVisionBridgeBaseUrl(config.model);

      // When routing through the RouteChi self-loop (non-standard provider),
      // keep the full provider-prefixed model ID so RouteChi can resolve the
      ...
      // Use sk-YOUR-API-KEY as fallback for self-loop if no API key is resolved.
      const selfLoopApiKey = resolvedApiKey || "sk-YOUR-API-KEY";
