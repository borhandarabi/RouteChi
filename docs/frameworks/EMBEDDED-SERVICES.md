...
Embedded services are locally-installed process sidecar tools that RouteChi installs, supervises, and
exposes as first-class routing targets. Unlike external providers (which are reached over the internet
via API keys), embedded services run on the same machine as RouteChi and communicate over loopback.
...
| --------------- | ----------------------------------------------- | :----------: | ------------------------------------------------------------------------------------------------------------------ |
| **9Router**     | `9router`                                      |    20130     | AI router that RouteChi can use as a sub-provider. Models exposed as `9router/{sub}/{model}`                     |
...
- RouteChi installs them under `DATA_DIR/services/{name}/` (isolated from RouteChi's own `package.json`)
- RouteChi spawns and monitors them as child processes
- RouteChi injects an ephemeral API key into the child's environment and rotates it without downtime (where applicable)
...
| API key management                    | RouteChi generates, encrypts at-rest (AES-256-GCM), and injects via env |
...
`package.json` and `node_modules/`. Does not conflict with RouteChi's own deps.
...
the next time RouteChi boots (if the service is installed).
...
- Strips incoming `cookie` and `authorization` headers (no leakage of RouteChi session)
...
2. Verify `npm` is in PATH: `which npm` from the same user account that runs RouteChi.
...
- `DATA_DIR` or its parent is not writable by the RouteChi process.
...
2. Ensure the RouteChi process user owns the directory: `chown -R $USER ~/.omniroute/`
...
Yes. Both services follow the same local-first model as RouteChi itself. They run
on the same machine and communicate over loopback. "Production" here means the VPS
or local server where RouteChi is deployed, not a remote cloud provider.
...
2. Check structured logs in RouteChi's pino output filtered by
