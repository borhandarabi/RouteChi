...
AgentBridge is RouteChi's MITM (Man-in-the-Middle) proxy that intercepts HTTPS traffic from IDE AI agents and reroutes it through RouteChi's unified routing engine. It supports **9 IDE agents** — Antigravity, Kiro, GitHub Copilot, OpenAI Codex, Cursor, Zed, Claude Code, Open Code, and Trae (investigating) — making RouteChi the broadest-coverage MITM proxy for AI coding assistants on the market.
...
- **Reroute any agent to any provider**: Copilot talking to OpenAI? Redirect it to Anthropic Claude, Gemini, or any of RouteChi's 226+ providers.
...
- **Apply RouteChi resilience**: combo routing, circuit breakers, fallbacks, and cost tracking work for IDE agent traffic too.
...
src/mitm/server.cjs  (port 443, CJS child process)
    ...
    ├── Target match? → fetch → RouteChi router (port 20128)
...
```bash
certutil -A -d sql:$HOME/.pki/nssdb -n "RouteChi AgentBridge" -t CT,, -i ~/.omniroute/mitm/ca.crt
```
...
AgentBridge intercepts credentials (OAuth tokens, API keys) that the IDE uses to authenticate with upstream providers. These are **masked before logging** (see §2.7) but are visible to RouteChi's MITM layer. First activation of each agent shows a dismissible risk notice modal.
...
The bypass list ensures that financial institutions, OAuth/SSO providers, and other sensitive hosts are **never decrypted**. Their TLS traffic passes through as a transparent TCP tunnel — RouteChi never sees the plaintext.
...
1. Verify the cert was installed: `security find-certificate -c "RouteChi AgentBridge"` (macOS) or `certutil -L -d sql:$HOME/.pki/nssdb` (Linux/NSS)
...
2. Check RouteChi server logs: `APP_LOG_LEVEL=debug` in `.env`
