...
**刚接触 RouteChi？** 从这里开始 —— 以下内容可解决 90% 的问题：

| 你看到的提示            | 含义                     | 解决方法                                                                                          |
| ----------------------- | ------------------------ | ------------------------------------------------------------------------------------------------- |
| "Can't connect"         | RouteChi 未运行          | 运行 `omniroute` 或 `docker restart omniroute`                                                     |
...
**原因：** 你使用的 Node.js 版本不在 RouteChi 批准的安全运行时范围内。最常见的情况是运行的 Node.js 22 或 24 补丁版本过低，未达到 RouteChi 所需的安全补丁基线。
...
3. 重新安装 RouteChi：`npm install -g routechi`
4. 重启：`omniroute`
...
**原因：** 执行全局 `npm install -g routechi` 后，包内的 `better-sqlite3` 原生二进制文件可能被编译为与你本地运行环境不同的架构或 Node.js ABI。这在 macOS 上很常见（Apple Silicon 和 Intel 均如此），当预编译的二进制文件与你的环境不匹配时就会发生。
...
**修复（v3.5.5+）：** RouteChi 现在在代理调度器激活时使用 undici 自身的 `fetch()` 函数，确保行为一致。请更新至 v3.5.5+。

### WSL 下的 MITM 代理：Windows 主机上的桌面应用未被拦截

**原因：** MITM 代理及其 CA 证书安装在 RouteChi 运行的环境中。在 WSL 下，该环境是 Linux 客户机，而 AI 桌面应用（Kiro、Trae、Copilot、Zed 等）运行在 Windows 主机上。主机应用不信任客户机的证书存储，也不会通过客户机的系统代理路由，因此桌面拦截不会在那里生效。

**建议：** 将 RouteChi 以原生方式运行在与你想要拦截的桌面应用相同的操作系统上（Windows 应用在 Windows 上运行；macOS/Linux 同理）。将 RouteChi 留在 WSL 内同时针对主机应用，需要手动在 Windows 主机上信任生成的 CA 证书，并将每个主机应用的网络/代理设置指向 WSL 代理端点 —— 这是一种不受支持、脆弱的配置方案。
...
当大量并发请求同时打到一个受速率限制的服务商时，RouteChi 使用互斥锁 + 自动速率限制来串行化请求，防止级联失败。这对 API 密钥类服务商是自动生效的。

---

## 可选的 RAG / LLM 失败分类法（16 种问题）

部分 RouteChi 用户将网关部署在 RAG 或代理栈之前。在这些场景中，经常会出现一种奇怪的现象：RouteChi 看起来一切正常（服务商在线、路由配置正常、无线速告警），但最终答案仍然不对。
...
如果你不在 RouteChi 后面运行 RAG 或代理流水线，可以忽略本节。
...
3. 重启 RouteChi 并从 **仪表盘 → CLI Tools** 重新测试
...
- 从 CLI 运行 `routechi providers` 重新触发 OAuth 流程，或
