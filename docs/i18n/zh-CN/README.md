...
- **🤖 一键 CLI/Agent 配置** — 专用 `setup-*` 命令为各编程工具一键配置 OmniRoute 路由（Claude Code、Codex、Cline、Continue、Cursor、Roo Code、Kilo Code、Crush、Goose、Qwen Code、Aider、OpenCode）；`routechi launch` / `routechi launch-codex` 为零配置启动器。→ [CLI 集成](../../guides/CLI-INTEGRATIONS.md)
- **🛰️ 远程模式** — 通过授权范围 Token 从任意机器操控远程 OmniRoute（`routechi connect` / `routechi contexts` / `routechi tokens`）；另附 `routechi login antigravity` 辅助命令，在你的本机运行 Google "native/desktop" OAuth 后将凭证 blob 粘贴至远程/VPS 安装实例（因远程环境无法接收 loopback 回调）。→ [远程模式](../../guides/REMOTE-MODE.md)
...
> 🎬 **制作了关于 OmniRoute 的视频？** 通过链接创建 [issue](https://github.com/borhandarabi/routechi/issues/new) 或 [discussion](https://github.com/borhandarabi/routechi/discussions) — 我们将在本节予以展示。
