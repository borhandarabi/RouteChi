...
> **خلاصه:** OmniRoute استفادهٔ توکن هر درخواست را پیگیری می‌کند، هزینه را محاسبه می‌کند،
...
هر درخواستی که از OmniRoute عبور می‌کند یک **سابقهٔ استفاده** تولید می‌کند که شامل موارد زیر است:
...
OmniRoute توکن‌ها را با استفاده از اکتشافی `~4 chars per token` **تخمین می‌زند**
(به `open-sse/services/autoCombo/pipelineRouter.ts` مراجعه کنید).

### توکن‌های کش‌شده

OmniRoute `cached_tokens` را جدا از `prompt_tokens` پیگیری می‌کند زیرا:
...
برای مدل‌های بدون دادهٔ قیمت‌گذاری، OmniRoute به **تخمین هزینه** با استفاده از نرخ‌های
