...
- **Contextها** هر کدام یک سرور را ذخیره می‌کنند (`~/.omniroute/config.json`، `chmod 600`).
  `routechi contexts use <name>` سرور فعال را تعویض می‌کند؛ `default` محلی است.
...
یک توکن با scope در داشبورد تولید کنید (یا با `routechi tokens create`) و آن را جای‌گذاری
...
`routechi configure` کاتالوگ مدل زندهٔ **سرور فعال** را می‌خواند و یک پیکربندی روی
**ماشین شما** می‌نویسد.
...
|--------------|-------|------------------|
| Codex        | `routechi setup-codex` | profileهای `~/.codex/<name>.config.toml` (به ازای مدل) |
| Claude Code  | `routechi setup-claude` | `~/.claude/profiles/<name>/settings.json` (به ازای مدل) |
| OpenCode     | `routechi setup-opencode` | `~/.config/opencode/opencode.json` — ارائه‌دهندهٔ openai-compatible با نام `omniroute` با هر مدل کاتالوگ (اجرای `opencode -m omniroute/<model>`) |
| Cline        | `routechi setup-cline` | `~/.cline/data/{globalState,secrets}.json` (حالت CLI) + چاپ تنظیمات افزونهٔ VS Code برای جای‌گذاری (سازگار با OpenAI، Base URL **بدون** `/v1`) |
| Kilo Code    | `routechi setup-kilo` | `~/.local/share/kilo/auth.json` (CLI) + تنظیمات `kilocode.*` در VS Code — سازگار با OpenAI، Base URL **با** `/v1` |
| Continue     | `routechi setup-continue` | `~/.continue/config.yaml` (VS Code/JetBrains + CLI مربوط به `cn`) — `provider: openai`، `apiBase` **با** `/v1`، کلید از طریق `${{ secrets.OMNIROUTE_API_KEY }}` |
| Cursor       | `routechi setup-cursor` | مراحل درون‌برنامه‌ای را چاپ می‌کند (Settings → Models → Override OpenAI Base URL **با** `/v1` + کلید + مدل). config مربوط به Cursor SQLite opaque است — فقط پنل چت |
| Roo Code     | `routechi setup-roo` | یک JSON import مربوط به Roo می‌نویسد (`~/.omniroute/roo-settings.json`) + تنظیم `roo-cline.autoImportSettingsPath` + چاپ مراحل رابط کاربری (سازگار با OpenAI، Base URL **با** `/v1`) |
| Crush        | `routechi setup-crush` | `~/.config/crush/crush.json` — ارائه‌دهندهٔ `openai-compat`، `base_url` **با** `/v1`، کلید از طریق `$OMNIROUTE_API_KEY` |
| Goose        | `routechi setup-goose` | `~/.config/goose/config.yaml` (`GOOSE_PROVIDER=openai` + `OPENAI_HOST` **بدون** `/v1` + `GOOSE_MODEL`) + دستورالعمل env |
| Qwen Code    | `routechi setup-qwen` | `~/.qwen/settings.json` — `modelProvider` با openai، `baseUrl` **با** `/v1`، کلید از طریق `envKey` (OMNIROUTE_API_KEY) |
| Aider        | `routechi setup-aider` | `~/.aider.conf.yml` (`openai-api-base` **بدون** `/v1` + `model: openai/<id>`) + دستورالعمل env (`aider --message --yes`) |
...
> OpenCode همچنین یک یکپارچه‌سازی **افزونه‌ای** غنی‌تر دارد: `routechi setup opencode`
...
یک **context** یک سرور ذخیره‌شده است (baseUrl + credential + scope). `routechi connect`
یکی ایجاد و آن را فعال می‌کند؛ از آن پس هر دستور به آن هدف می‌گیرد. با `routechi contexts`
...
> بردن واقعی دسترسی، توکن را روی سرور با `routechi tokens revoke <id>` ابطال کنید.
...
- `routechi connect` از قفل brute-force ورود و ثبت لاگ ممیزی مجدد استفاده می‌کند.
