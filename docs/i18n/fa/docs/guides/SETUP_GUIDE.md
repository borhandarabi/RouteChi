...
| `routechi setup`       | onboarding تعاملی CLI برای گذرواژه و اولین ارائه‌دهنده          |
| `routechi doctor`      | اجرای بررسی‌های سلامت محلی بدون راه‌اندازی سرور            |
| `routechi providers`   | کشف، فهرست، اعتبارسنجی و تست ارائه‌دهندگان از CLI          |
...
| `routechi --no-open`   | مرورگر به‌صورت خودکار باز نشود                                        |
| `routechi --help`      | نمایش راهنما                                                      |
...
routechi setup-opencode     # ~/.config/opencode/opencode.json (سازگار با openai)
routechi setup-cline        # تنظیمات Cline CLI + افزونهٔ VS Code
routechi setup-kilo         # Kilo Code
routechi setup-continue     # ~/.continue/config.yaml (Continue / cn)
routechi setup-cursor       # مراحل درون‌برنامه‌ای Cursor را چاپ می‌کند
routechi setup-roo          # import Roo Code + pointer autoImport
