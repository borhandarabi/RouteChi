---
title: "🐳 راهنمای Docker — OmniRoute"
version: 3.8.40
lastUpdated: 2026-06-28
---

# 🐳 راهنمای Docker — OmniRoute
...
OmniRoute چهار پروفایل Compose ارائه می‌دهد. آنچه با محیط شما سازگار است را انتخاب کنید.
...
| `cli`           | `omniroute-cli`  | گردش‌کاری‌های agentic که `omniroute providers/setup/doctor` و CLIهای همراه (Codex، Claude Code، Droid، OpenClaw) را فراخوانی می‌کنند | `docker compose --profile cli up -d`        |
...
OmniRoute برای پشتیبانی از rate limiter توزیع‌شده و کش مشترک به Redis وابسته است. سرویس
...
OmniRoute می‌تواند به‌طور امن با استفاده از تأمین SSL خودکار Caddy نمایان شود. مطمئن شوید
...
Caddy هدرهای استاندارد forwarding را برای کانتینر بالادست تنظیم می‌کند. OmniRoute از
...
پیشرفته‌ای که عمداً می‌خواهید OmniRoute origin عمومی را از هدرهای forwarding قابل‌اعتماد
...
- Quick Tunnelها پس از راه‌اندازی مجدد OmniRoute یا کانتینر به‌طور خودکار بازگردانده نمی‌شوند.
...
- اگر می‌خواهید OmniRoute به‌جای دانلود، از یک باینری موجود استفاده کند، `CLOUDFLARED_BIN=/absolute/path/to/cloudflared` را تنظیم کنید.

## تگ‌های ایمیج

| ایمیج                   | تگ       | حجم    | توضیح               |
| ----------------------- | -------- | ------ | ------------------- |
| `diegosouzapw/omniroute` | `latest` | ~250MB | آخرین نسخهٔ پایدار  |
| `diegosouzapw/omniroute` | `3.8.0`  | ~250MB | نسخهٔ فعلی          |
...
- **حالت WAL مربوط به SQLite:** باید اجازه داده شود `docker stop` به پایان برسد تا OmniRoute
