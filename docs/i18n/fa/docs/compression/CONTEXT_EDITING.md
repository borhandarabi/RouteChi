...
موتورهای فشرده‌سازی محلی OmniRoute (Caveman، RTK، LLMLingua، خطوط لوله ترکیبی) — که بدنه درخواست را
_قبل_ از خروج از پروکسی بازنویسی می‌کنند — Context Editing از **provider** می‌خواهد بلوک‌های قدیمی
tool-use / tool-result را از پنجره بافتار در حال اجرای خود پاک کند. OmniRoute فقط یک پارامتر بدنه
(`context_management.edits[]`) اضافه می‌کند؛ Claude پاک‌سازی واقعی را در برابر توکنایزر خودش انجام می‌دهد.

این قابلیت بر اساس ماهیت واگذارشده است: providerهای دیگر این پارامتر را رد می‌کنند، بنابراین OmniRoute
...
OmniRoute یک ویرایش واحد در بدنه خروجی Anthropic Messages تزریق می‌کند:
...
این قابلیت بتا از طریق هدر `anthropic-beta: context-management-2025-06-27` اعلام می‌شود که OmniRoute
...
دست‌نخورده باقی می‌ماند. اگر یک ویرایش `clear_thinking_20251015` نیز وجود داشته باشد، OmniRoute
...
پس از یک پاسخ Claude، OmniRoute ثبت می‌کند که provider چقدر بافتار را واقعاً پاک کرده است. این کار
...
| ----------------- | --------------------------------------------------- | ------------------------------------------- |
| محل اجرا          | در OmniRoute، قبل از خروج درخواست از پروکسی          | در provider (Claude)، سمت سرور              |
...
این دو مکمل یکدیگرند: موتورهای محلی بایت‌هایی که OmniRoute ارسال می‌کند را فشرده می‌کنند؛ Context Editing
